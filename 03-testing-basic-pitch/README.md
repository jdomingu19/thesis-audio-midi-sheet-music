...
- `<save-midi>`
  - _bool_ to control generating and saving a MIDI file to the `<output-directory>`
- `<sonify-midi>`
  - _bool_ to control saving a WAV audio rendering of the MIDI file to the `<output-directory>`
- `<save-model-outputs>`
  - _bool_ to control saving the raw model output as a NPZ file to the `<output-directory>`
- `<save-notes>`
  - _bool_ to control saving predicted note events as a CSV file `<output-directory>`
- `<model-path>`
  - _str_ or _pathlib.Path_ local path from where to load the model, can eg: use the path obtained with `from basic_pitch import ICASSP_2022_MODEL_PATH`
