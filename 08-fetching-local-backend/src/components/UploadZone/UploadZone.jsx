...
const SUPPORTED_FORMATS = ['MP3', 'WAV', 'OGG', 'FLAC', 'M4A', 'WebM']
const ACCEPTED_EXTENSIONS = '.mp3,.wav,.ogg,.flac,.m4a,.webm,.aac,.opus'

export default function UploadZone({ onFilesAdded, error, onErrorDismiss }) {
  ...
  const handleDragEnter = useCallback((e) => {
    ...
    setDragState(hasAudio || (e.dataTransfer.items?.length ?? 0) === 0 ? 'over' : 'reject')
  }, [])
    ...
    <section className={styles.wrapper}>
          ...
          {dragState === 'reject' ? (
            ...
            <>
              <p className={styles.primaryText}>
                ...
                <span className={styles.browseLink}>selecciona desde tu dispositivo</span>
              </p>
              <p className={styles.secondaryText}>Hasta 25 MB por archivo</p>
            </>
          )}
        </div>
      ...
      {error && (
        <div className={styles.errorBanner} role="alert">
          <ErrorIcon />
          <span>{error}</span>
          <button className={styles.errorDismiss} onClick={onErrorDismiss} aria-label="Cerrar error">
            ✕
          </button>
        </div>
      )}
...
function UploadIcon({ active }) {
  return (
    <svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={active ? 2.5 : 1.5} strokeLinecap="round" strokeLinejoin="round">
