    ...
    <article className={styles.card} style={{ '--accent': accentColor }}>
      ...
      <div className={styles.content}>
        <div className={styles.topRow}>
          <p className={styles.filename} title={file.name}>{truncateName(file.name)}</p>
        ...
        <div className={styles.timeRow}>
          ...
          {hasError && <span className={styles.errorText}>Error al cargar audio</span>}
          {isFailed && <span className={styles.errorText}>{file.errorMessage || 'Fallo la conversión'}</span>}
        ...
        {!isDone && (
          ...
          >
            ...
            <span className={styles.convertLabel}>
              {isConverting ? 'Procesando…' : isFailed ? 'Reintentar' : 'Convertir'}
            </span>
          </button>
        )}
...
function LoadingSpinner() {
  return (
    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" className={styles.spinner}>
